...
- `packages/parser/src/post-process.ts` — post-parse transforms: don't-care variable (`_`) desugaring, numeric literal `rawText` preservation, aggregate `FunctionCall → AggregateCall` rewrite in rule heads, `BracketAccess` splitting into `Subscript` or `Slice` based on whether a `:` was present (the grammar parses a single `BracketAccess` to avoid an LL(k) ambiguity), and the whole **proof-term / ADT desugar** (proof-column injection, constructor-term matching, capture shorthand) which lowers onto the existing `value` machinery. The `mod` keyword is left as-is in the AST; the translator emits SQL `%` at codegen time. Optional **head type annotations** (`h(x: integer)`) are handled separately by `liftHeadAnnotations`, run in `parseRaw` (not `postProcess`) so no later stage sees an `AnnotatedHeadTerm`: it strips each wrapper and records the declared type on `head.argTypes` (undefined per unannotated position; array absent when the rule has none).
