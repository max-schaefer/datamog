...
The native evaluator (`packages/backend/native/src/values.ts`, `planner.ts`) is the *authoritative* runtime spec — when the native and SQL emissions disagree, the native is usually right (cross-checked against the spec).

## High-yield veins (in priority order)

### 1. Cross-backend operator parity on edge inputs

For each runtime operator / built-in, walk the matrix `<spec promise> × <native impl> × <SQL emission> × <per-dialect override>`. Anywhere two cells diverge, one is a bug. Edge inputs that have repeatedly surfaced bugs:
...
When you fix one edge-case parity bug, **immediately grep for parallel sites** — the slice fix follows the subscript fix, the boolean `concat` fix surfaces the `value` `concat` fix.

### 2. Loader-side validation drift from native `as_*.value`

The native `as_*.value` builtins (`packages/backend/native/src/values.ts`) are the type-validation spec. `coerceValue` and `checkValue` in `packages/engine/src/loader.ts` should match them line-for-line. Drift has been found three times: integer (`isInteger` → `isSafeInteger`, `f7f67e8`), float (`typeof === "number"` → `Number.isFinite`, `6543cf6`), value (no validation → `isJsonValue` rejects non-finite numbers, `bb612e9`). When auditing a new validator, paste both implementations side by side.
...
`inferTypes` in `packages/core/src/types.ts` validates rule bodies and query (`?-`) bodies with two SEPARATE but near-identical `switch (elem.$type)` loops (`Literal` / `Equality` / `Filter` / `RangeAtom`). They drift: the query-body `Equality` case once omitted the `checkComparableTypes` call the rule case makes, and the query-body `Filter` case carried an extra `&& t !== "value"` exemption the rule case lacks — so a `?-` query was accepted where the equivalent rule was rejected (`2170d1a`). When auditing any per-element validation, diff the query loop against the rule loop case-by-case; the same `analyze`/`inferTypes` logic should accept/reject a body element identically whether it sits in a rule or a query. The general shape (catalog pattern 1, "two parallel implementations of the same thing drift apart") recurs across this codebase's near-duplicate paths: rule-vs-query (analyzer, translator SQL emission, native projection), the loader trio (`directory-loader` / vscode `disk-loader` / playground in-memory), and the VS Code validator vs playground worker (vein 6). To test: construct the same construct in both halves and confirm identical accept/reject (use `inferTypes(analyze(parse(src)))`, NOT `analyze()` alone — type checks live in `inferTypes`).
...
- **Storage-vs-runtime gate drift** (a vein-2 instance): `checkValue` (JSONL/JSON load) copied the runtime `as_integer` 2^53 bound, but EDB integer columns are int4 on Postgres, so it accepted integers `coerceValue` (CSV) rejects. Loader gates enforce the STORAGE bound (int4, 9 digits), not the runtime one.

To hunt: run the same program on sqlite AND native (the `executeOnSqliteAndNative` helper in `packages/engine/test/executor.test.ts`) over projecting IDBs feeding aggregates, chained/nested `value` access, `to_json`/`concat` of numeric-leaf values, and out-of-int4 integers per format.
...
Use this pattern liberally — it's faster than reading the agent's analysis cover-to-cover, and it kills false positives outright. For cross-backend parity claims, instantiate two backends in the same script and diff their outputs:
...
- **`to_string(float)` of an integer-valued float** — `"1"` on Postgres / native, `"1.0"` on SQLite / sql.js. Documented and accepted in `doc/spec.md` (search "Cross-backend variance"). Don't try to "fix" the divergence; users wanting bit-identical output should `to_string(round(x))`. **Same variance class extends to JSON literals AND `parse_json` of integer-valued floats** — `[1.0]`/`[1]` and `parse_json("3.0")`/`3` are textually identical on Postgres (`jsonb` canonicalises numerically) and on native (JS `Number` collapses `1.0` to `1`), but textually distinct on SQLite / sql.js, which keeps the `.0`. This has real *consequences* beyond display — `parse_json("3.0") = 3` is `true` on native/Postgres but `false` on SQLite, and `r(X) :- X = 3.0. r(X) :- X = parse_json("3.0").` deduplicates to 1 row on native/Postgres but 2 on SQLite — yet it is the SAME accepted v1 variance and sits on a genuine spec ambiguity (the storage note promises number normalisation while the explicit "Cross-backend variance" note accepts `1.0` vs `1`). Multiple bug-hunt agents independently rediscover this each session via `as_integer`/`concat`/equality/dedup on integer-valued floats. **Don't fix autonomously** — same class as string-subscript-OOB below: resolve the spec contradiction with the maintainer first. (The genuinely-fixable sibling is `as_integer(3.0)` returning the integer 3 — a *semantic extraction* whose result is spec-defined regardless of textual storage; that was a real bug, `ed46a4a`. The line is: textual/dedup divergence of the stored value = accepted variance; a value-producing builtin returning the wrong answer = bug.)
- **`parse_json` of an object literal with non-sorted keys** — SQLite's `json()` minifies but doesn't sort keys, so `parse_json('{"b":2,"a":1}')` and `parse_json('{"a":1,"b":2}')` are textually distinct on SQLite/sql.js but unify on Postgres / native. Documented variance in `doc/spec.md:702-706`. **Important contrast with the literal path** (which IS canonicalised, see vein 1): when a `parse_json` value is nested *inside* an object literal — `{"data": parse_json(s)}` — the outer object's keys are sorted by the dialect, but the inner `parse_json` value retains its source key order. The variance is bounded to the inner value but doesn't recursively contaminate everything around it.
- **String subscript out-of-range returning `""`** — `doc/spec.md` §2.6 says NULL but §5.4 says backend-defined. The implementation (both native and SQL) consistently returns `""`. Spec ambiguity, not a bug. Don't change without resolving the spec contradiction first.
- **Backend persistence between `executor.execute()` calls** — a long-lived shared backend would leak tables/views across unrelated programs (`CREATE TABLE IF NOT EXISTS` doesn't drop). The shipped consumers (CLI, playground worker) all create a fresh backend per execution and `close()` in `finally`, so this isn't reachable from realistic usage.
...
- **Strings with an embedded NUL (`U+0000`) diverging on `length`/subscript/slice** — reachable via `parse_json("\"\\u0000\"")`. Native/seminaive count code points; SQLite/sql.js `LENGTH`/`SUBSTR` stop at the NUL (C-string terminator); PostgreSQL `text`/`jsonb` cannot store `U+0000` at all, so three-backend parity is unreachable. Documented as a non-portable-input limitation in `doc/spec.md` §2.6 (the subscript/slice "Cross-backend variance" note), alongside the ±2^53 and deep-`parse_json` ceilings. Don't re-file the `length("a b")` = 0-on-sqlite / 2-on-native divergence as a new bug.
- **Deep chained `value` access built directly on `parse_json` overflowing SQLite's parser** — `parse_json` emits a large recursive-CTE / nested-CASE SQL that already sits near SQLite's parser stack limit, so a chain like `parse_json(s)[0][1:3]` can still overflow even after the receiver-duplication fix (`e4892b8`) made subscript/slice grow linearly. Chains on ordinary receivers (value columns, object/array literals) work to a much greater depth. Reducing `parse_json`'s own SQL depth is a separate, larger change; don't file the residual parse_json-chain overflow as a new receiver-duplication bug.
- **`parse_json("null")` returning SQL NULL** — intentional under the current runtime model (`2d7157a`). Native represents SQL NULL and JSON null with the same JS `null`, and SQL backends collapse JSON null at expression boundaries for parity. Preserving a distinguishable JSON null would require a `Value` representation change; in the current model, the bug is a backend accidentally leaking `"null"` / `jsonb 'null'` as a live value.
