...
The loader looks for `<predicate>.mmd` in the configured directory (e.g. `data/edge.mmd` for an `input predicate edge(...)` declaration). Predicates with 2 columns (source, target) or 3 columns (source, target, label) are supported. Edge labels are extracted from the `-->|label|` syntax; edges without labels get an empty string.
...
```
input predicate road(from: string, to: string, weight: string).
```
