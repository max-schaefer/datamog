...
Datamog compiles this to a SQL `RECURSIVE VIEW` that iterates until no new
...
```datalog
ancestor(X, Y) :- parent(X, Y).
ancestor(X, Y) :- ancestor(X, Z), ancestor(Z, Y).     % double-recursive
```

All three produce the same results. Datamog (and SQL) handles all of them via
`WITH RECURSIVE`. In some Datalog systems the choice affects performance, but
the logical meaning is identical.
